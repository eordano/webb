...
export function importPrivateKeyFromHexString(privateKeyAsHexadecimalString: string): CryptographicIdentity {
  ...
  if (!HEXADECIMAL_REGEXP.test(privateKeyAsHexadecimalString)) {
    throw new Error(`An invalid private key was provided: ${privateKeyAsHexadecimalString}: it should be in hexadecimal format`);
  }
  ...
  if (!privateKeyVerify(privateKey)) {
    throw new Error(`An invalid private key was provided: ${privateKeyAsHexadecimalString}: it is not a valid value for the secp256k1 curve`);
  }
