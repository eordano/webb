export default function validateCommands(commands: any[], helpFn?: Function, clearFn?: Function, noDefaults?: boolean) {
  ...
  for (const c in commands) {
    ...
    if (!noDefaults && immutables.includes(c)) {
      throw new Error(
        `Attempting to overwrite default command '${
          immutables[immutables.indexOf(c)]
        }'; cannot override default commands when noDefaults isn't set`
      )
    }
