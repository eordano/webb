...
export default class Terminal extends Component<any, any> {
  ...
  validateCommands() {
    const validCommands = validateCommands(this.props.commands, this.showHelp, this.clearStdout, this.props.noDefaults)
  ...
  showWelcomeMessage() {
    ...
    if (typeof msg === 'boolean')
      this.pushToStdout(`Welcome to the React terminal! Type 'help' to get a list of commands.`)
    else if (Array.isArray(msg)) msg.forEach(item => this.pushToStdout(item))
  ...
  pushToStdout(message: any, rawInput?: any) {
  ...
  render() {
    ...
    return (
      ...
      >
        {/* Content */}
        <div className={this.props.contentClassName} style={styles.content}>
          {/* Stdout */}
          {this.getStdout()}
          {/* Input area */}
          <div className={this.props.inputAreaClassName} style={styles.inputArea}>
            {/* Prompt label */}
            <span className={this.props.promptLabelClassName} style={styles.promptLabel}>
