...
import { rendererSentLoaded, sceneLoading, sceneRendererError, sceneRunning, SCENE_STOP, scriptSentAwake, scriptTimedout } from './actions'
...
import { sceneManager, watchScriptForAwake, watchRendererForLoaded, watchForSceneDispose } from './sagas'
  ...
  if (shouldtrigger) {
    ...
    return yield spawn(function* racer() {
      ...
      const result = (yield race({
        ...
        stop: take((action: any) => action && action.type === SCENE_STOP && (action as StopScene).payload.sceneId === scene.id)
      })) as any;
      if (result.stop) {
        worker.dispose();
      }
